...
<ul id="keyboard">
    ...
    {#each notes as key, index }
      <li id={key} class={`${key.includes("#") ? "black" : "white"}`}>{key}</li>
...
#keyboard {
  ...
  box-shadow: 0 0 50px var(--black-50) inset, 0 1px var(--keyboard-shadow) inset,
    ...
    <script>
        ...
        import {addEventListners, getKeyboardLayout, makePianoNotes} from "../../utils/helpers"
        // @ts-ignore
import * as Tone from 'tone'

const {keys, notes} = getKeyboardLayout(0, 90, 0, 90)
